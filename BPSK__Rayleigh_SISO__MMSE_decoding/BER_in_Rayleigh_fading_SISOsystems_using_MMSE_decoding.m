...
for k = 1:length(snrdB_list)
    ...
    for n = 1:nSim
        ...
        nTx = 1; % the number of transmit antennas
        nRx = 1; % the number of receive antennas
        positive_lambda = nTx/snr; % we can choose another positive value for possitive_lambda
        matrix = h' * (h*h' + positive_lambda*eye(nRx))^-1;
        xhat = matrix*y; % estimates the transmitted signal
...
xlabel('snr (dB)');
ylabel('BER');
title('BPSK mod. & Rayleigh channel & SISO system & MMSE decoding');