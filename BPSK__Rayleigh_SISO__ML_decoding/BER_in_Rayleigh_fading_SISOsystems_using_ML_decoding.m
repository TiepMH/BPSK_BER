...
constellation_points = [-1, 1]; % BPSK
nConstellationPoints = 2; %BPSK means 2-PSK
...
for k = 1:length(snrdB_list)
    ...
    for n = 1:nSim
        ...
        distances = zeros(1, nConstellationPoints);
        for arg = 1:nConstellationPoints
            x_possible = constellation_points(arg);
            distance = norm(y - h*x_possible)^2;
            distances(arg) = distance;
        end
        [min_val, arg_min] = min(distances);
        xhat = constellation_points(arg_min); % estimates the transmitted signal
...
xlabel('snr (dB)');
ylabel('BER');
title('BPSK mod. & Rayleigh channel & SISO system & ML decoding');
